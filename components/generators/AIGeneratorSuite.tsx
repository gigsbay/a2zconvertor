...
type Availability = { freeTrialEnabled: boolean; freeDailyLimit: number; freeGenerationsRemaining: number };

export default function AIGenerator({ toolSlug }: { toolSlug: string }) {
  const config = configs[toolSlug];
  const [availability, setAvailability] = useState<Availability>({ freeTrialEnabled: true, freeDailyLimit: 3, freeGenerationsRemaining: 3 });
  ...
  useEffect(() => {
    const timer = window.setTimeout(() => {
      fetch("/api/ai/generate").then((response) => response.ok ? response.json() : Promise.reject()).then((data: Availability) => setAvailability(data)).catch(() => setAvailability((current) => ({ ...current, freeTrialEnabled: false, freeGenerationsRemaining: 0 })));
    }, 0);
  ...
  async function generate(event: FormEvent) {
    ...
    if (!availability.freeTrialEnabled) { setError("Free AI generation is temporarily unavailable. Please try again later."); return; }
    ...
    try {
      ...
      if (!response.ok) {
        if (data.code === "DAILY_LIMIT_REACHED") setLimitReached(true);
        if (typeof data.freeGenerationsRemaining === "number") setAvailability((current) => ({ ...current, freeGenerationsRemaining: data.freeGenerationsRemaining }));
        throw new Error(data.error || "Generation failed.");
      }
      setResult(data.result);
      setLimitReached(data.freeGenerationsRemaining <= 0);
      setAvailability((current) => ({ ...current, freeGenerationsRemaining: data.freeGenerationsRemaining }));
    } catch (cause) { setError(cause instanceof Error ? cause.message : "Generation failed."); }
    finally { setLoading(false); }
  }

  async function copy(value: string) { await navigator.clipboard.writeText(value); setCopied(value); window.setTimeout(() => setCopied(""), 1500); }
  ...
  return <div className="rounded-3xl border border-white/10 bg-slate-900/70 p-6 sm:p-8">
    <div className="flex flex-wrap items-start justify-between gap-4"><div>
      <span className="inline-flex rounded-full border border-emerald-400/30 bg-emerald-400/10 px-3 py-1 text-sm font-bold text-emerald-300">Free AI {"\u00b7"} {availability.freeDailyLimit}/day</span>
      <h1 className="mt-4 text-4xl font-black">{config.title}</h1>
      <p className="mt-4 max-w-2xl text-slate-400">Free AI tools powered by Gemini. You get {availability.freeDailyLimit} free generations per day. No account or API key is required.</p>
    ...
    <form onSubmit={generate} className="mt-8 grid gap-4 md:grid-cols-2">
      ...
      <button disabled={loading || limitReached || !availability.freeTrialEnabled} className="rounded-xl bg-blue-600 px-5 py-3 font-semibold text-white disabled:cursor-not-allowed disabled:opacity-50 md:col-span-2">{loading ? "Generating with Gemini..." : result ? "Regenerate" : "Generate"}</button>
    </form>
    <p className="mt-4 text-sm text-slate-400">You have {availability.freeGenerationsRemaining} free AI generations left today.</p>
