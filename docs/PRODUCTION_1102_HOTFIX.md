...
- `/convert/[slug]` was still dynamic, so every tool page could render through the Worker at request time.
...
- Forced static rendering for static pages and pre-renderable dynamic routes.
- Added `generateStaticParams` for all converter pages.
- Added `dynamicParams = false` for converter, resource and comparison dynamic routes.
