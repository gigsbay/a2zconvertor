...
- `AI_FREE_DAILY_LIMIT=10`: successful generations allowed per hashed user identifier per UTC day.
...
Traffic spikes, automated abuse, prompt length and provider pricing can change cost. Keep the default limit at 10, monitor Gemini usage, set provider quotas and disable the feature immediately if usage is unexpected.
